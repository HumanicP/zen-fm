...
it('updates favorites when Books is renamed to Books1 in mock mode', async () => {
  ...
  const favorites = ['/Books', '/Books/Novel', '/Bookshelf', '/']
  expect((await invoke(middleware, 'PUT', '/api/v1/settings', { theme: 'dark', favorites })).status).toBe(200)

  const failed = await invoke(middleware, 'POST', '/api/v1/files/move', { source: '/Books', destination: '/Documents' })
  expect(failed.status).toBe(400)
  expect(JSON.parse((await invoke(middleware, 'GET', '/api/v1/settings')).body)).toMatchObject({ favorites })
  ...
  expect(JSON.parse((await invoke(middleware, 'GET', '/api/v1/settings')).body)).toMatchObject({
    favorites: ['/Books1', '/Books1/Novel', '/Bookshelf', '/'], theme: 'dark',
  })
